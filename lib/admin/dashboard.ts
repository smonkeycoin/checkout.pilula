...
export function buildDashboardData(input: DashboardInput) {
  ...
  const paidOrders = input.orders.filter((order) => isPaidOrderStatus(order.status));
  const pendingOrders = input.orders.filter((order) => isPendingOrderStatus(order.status));
  ...
  const doctorsConfirmed = paidOrders.filter((order) => order.profile_type === "doctor").length;
  const patientsConfirmed = paidOrders.filter((order) => order.profile_type === "patient").length;

  const approvedInvites = input.invites.filter((invite) => invite.status === "approved" || invite.status === "opened" || invite.status === "paid" || Boolean(invite.approved_at));
  const openedInvites = input.invites.filter((invite) => invite.status === "opened" || invite.status === "paid" || Boolean(invite.opened_at));
  const expiredInvites = input.invites.filter((invite) => isExpiredInvite(invite, now));
  const revokedInvites = input.invites.filter((invite) => invite.status === "revoked" || Boolean(invite.revoked_at));
  const paidInviteCount = input.invites.filter((invite) => invite.status === "paid" || paidInviteIds.has(invite.id)).length;
  ...
  const recentPayments = input.orders
    .slice()
  ...
  const activities = [
    ...input.invites.map((invite) => makeActivity("invite_created", "payment_invite", invite.id, "Invitación creada", invite.created_at)),
    ...input.invites.filter((invite) => invite.opened_at).map((invite) => makeActivity("invite_opened", "payment_invite", invite.id, "Invitación abierta", invite.opened_at)),
    ...input.invites.filter((invite) => invite.revoked_at).map((invite) => makeActivity("invite_revoked", "payment_invite", invite.id, "Invitación revocada", invite.revoked_at)),
    ...input.otps.filter((otp) => otp.verified_at && otp.invite_id).map((otp) => makeActivity("otp_verified", "payment_invite", String(otp.invite_id), "OTP verificado", otp.verified_at)),
    ...input.orders.map((order) => makeActivity("checkout_started", "order", order.id, `Checkout iniciado ${order.reference || ""}`.trim(), order.created_at)),
  ...
  const paymentMethods = {
    card: {
      count: input.orders.filter((order) => order.payment_method === "card").length,
      mxn: input.orders.filter((order) => order.payment_method === "card" && order.currency === "mxn").reduce((sum, order) => sum + cents(order.amount_total), 0),
      usd: input.orders.filter((order) => order.payment_method === "card" && order.currency === "usd").reduce((sum, order) => sum + cents(order.amount_total), 0)
    },
    spei: {
      count: input.orders.filter((order) => order.payment_method === "bank_transfer").length,
      mxn: input.orders.filter((order) => order.payment_method === "bank_transfer" && order.currency === "mxn").reduce((sum, order) => sum + cents(order.amount_total), 0),
      usd: input.orders.filter((order) => order.payment_method === "bank_transfer" && order.currency === "usd").reduce((sum, order) => sum + cents(order.amount_total), 0)
    },
    currency: {
      mxn: { count: input.orders.filter((order) => order.currency === "mxn").length, amount: input.orders.filter((order) => order.currency === "mxn").reduce((sum, order) => sum + cents(order.amount_total), 0) },
      usd: { count: input.orders.filter((order) => order.currency === "usd").length, amount: input.orders.filter((order) => order.currency === "usd").reduce((sum, order) => sum + cents(order.amount_total), 0) }
    }
  };
  ...
  return {
    ...
    paymentMethods,
    integrations: {
      stripe: isPlaceholder(env.STRIPE_SECRET_KEY) ? "pending" : isLiveStripeKey(env.STRIPE_SECRET_KEY) ? "operational" : "test_mode",
